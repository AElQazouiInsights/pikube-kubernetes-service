...
> [!IMPORTANT]
>
> *Before going further, [**`Node Pre-configuration`**](../4-kubernetes/1-k3s-installation.md#1-node-pre-configuration) need to be completed.*
...
```bash
sudo kubectl apply -f k3s-server-upgrade.yaml -f k3s-agent-upgrade.yaml
```
